<?php 
/**
 * Tmpdata Page Controller
 * @category  Controller
 */
class TmpdataController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'tmpdata';
		$fields = array('id', 
			'tgl', 
			'ket', 
			'mutasi', 
		...
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			...
			$db->orderBy('tmpdata.id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Tmpdata";
		$this->view->render('tmpdata/list.php' , $data ,'main_layout.php');
	}
// No View Function Generated Because No Field is Defined as the Primary Key on the Database Table
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'tmpdata';
			$fields = $this->fields = array('id','tgl','ket','mutasi','mkode','userbca'); //insert fields
		...
		$this->view->page_title ="Add New Tmpdata";
		$this->view->render('tmpdata/add.php' ,null,'main_layout.php');
	}
