<?php 
/**
 * Detailbca Page Controller
 * @category  Controller
 */
class DetailbcaController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'detailbca';
		$fields = array('id', 
			'tgl', 
			'ket', 
			'mutasi', 
		...
		if(!empty($this->search)){
			...
			$db->where("(id LIKE ? OR tgl LIKE ? OR ket LIKE ? OR mutasi LIKE ? OR mkode LIKE ? OR userbca LIKE ?)", array("%$text%","%$text%","%$text%","%$text%","%$text%","%$text%"));
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('detailbca.id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Mutasi BCA";
		$this->view->render('detailbca/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('detailbca.id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Detailbca";
			$this->view->render('detailbca/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		...
		$this->view->page_title ="Add New Detailbca";
		$this->view->render('detailbca/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'detailbca';
		$fields = $this->fields = array('id','tgl','ket','mutasi','mkode','userbca'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Detailbca";
		if(!empty($data)){
			$this->view->render('detailbca/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('detailbca.id' , $rec_id,"=",'OR');
		}
