<?php 
/**
 * Kelompok Page Controller
 * @category  Controller
 */
class KelompokController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'kelompok';
		$fields = array('kd_kelompok', 
			'nama_kelompok', 
			'kuota_total', 
			'kuota_terisi', 
			'kuota_tersedia', 
		...
		if(!empty($this->search)){
			...
			$db->where("(kd_kelompok LIKE ? OR nama_kelompok LIKE ? OR kuota_total LIKE ? OR kuota_terisi LIKE ? OR kuota_tersedia LIKE ? OR kd_akun LIKE ? OR status LIKE ?)", array("%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%"));
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('kelompok.kd_kelompok', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Kelompok";
		$this->view->render('kelompok/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('kelompok.kd_kelompok' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Kelompok";
			$this->view->render('kelompok/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'kelompok';
			$fields = $this->fields = array('nama_kelompok','kuota_total','kd_akun','status'); //insert fields
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
				'nama_kelompok' => 'required',
				'kuota_total' => 'required|numeric',
			);
			$this->sanitize_array = array(
				'nama_kelompok' => 'sanitize_string',
				'kuota_total' => 'sanitize_string',
			);
...
$modeldata['status']="active";
		...
		$this->view->page_title ="Add New Kelompok";
		$this->view->render('kelompok/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'kelompok';
		$fields = $this->fields = array('kd_kelompok','nama_kelompok','kuota_total','kd_akun','status'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
...
$modeldata['status']="active";
		...
		$this->view->page_title ="Edit  Kelompok";
		if(!empty($data)){
			$this->view->render('kelompok/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('kelompok.kd_kelompok' , $rec_id,"=",'OR');
		}
