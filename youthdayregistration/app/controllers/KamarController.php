<?php 
/**
 * Kamar Page Controller
 * @category  Controller
 */
class KamarController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'kamar';
		$fields = array('kd_kamar', 
			'nama_kamar', 
			'jenis_kelamin', 
			'kuota_total', 
			'kuota_terisi', 
		...
		if(!empty($this->search)){
			...
			$db->where("(kd_kamar LIKE ? OR nama_kamar LIKE ? OR jenis_kelamin LIKE ? OR kuota_total LIKE ? OR kuota_terisi LIKE ? OR kuota_tersedia LIKE ? OR kd_akun LIKE ? OR active LIKE ?)", array("%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%"));
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('kamar.kd_kamar', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Kamar";
		$this->view->render('kamar/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('kamar.kd_kamar' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Kamar";
			$this->view->render('kamar/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'kamar';
			$fields = $this->fields = array('nama_kamar','jenis_kelamin','kuota_total','kd_akun','active'); //insert fields
...
$modeldata['active']="active";
		...
		$this->view->page_title ="Add New Kamar";
		$this->view->render('kamar/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'kamar';
		$fields = $this->fields = array('kd_kamar','nama_kamar','jenis_kelamin','kuota_total','kd_akun','active'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
...
$modeldata['active']="active";
		...
		$this->view->page_title ="Edit  Kamar";
		if(!empty($data)){
			$this->view->render('kamar/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('kamar.kd_kamar' , $rec_id,"=",'OR');
		}
